...
> **Status note:** the Day 87-92 feature freeze (paper-trading accumulation as the sole priority) is **lifted as of Day 113** — the automated paper-trading program was discontinued entirely at the user's explicit direction ("I want to use Claude and STA as analysis engine and recommendation, I will take care of stuff myself"). STA is now a pure analysis/recommendation tool; the user places and manages all real trades themselves. See [Roadmap](#roadmap) for current priorities.
...
10. **Sector Rotation** (Day 58-62 - v4.19 → v4.24, redesigned Day 93 - v4.50, Sub-Industry Watch Day 101 - v4.55, Sector Pullback Screener Day 103 - v4.56, Sub-Sector Pullback Screener Day 105 - v4.57)
    ...
    - **Sector Pullback Screener** — `/api/sectors/pullback-screen`, a discretionary screen (NOT an automated signal) surfacing strong names in Improving-quadrant sectors pulling back to their 21-EMA. Backed by a mechanical rule set that was fully backtested (survivorship-free, 2020-2025) and failed its own pre-registered bar (34.1% win rate, PF 1.177) — so it's shown purely as candidates for the user's own judgment, with a permanent disclaimer, not a live forward-test track. Collapsible section on the Sectors tab (Day 103)
    - **Sub-Sector Pullback Screener** — `/api/sectors/sub-industry-pullback-screen`, the same discretionary screen applied to the 21 Sub-Industry Watch clusters instead of the 11 broad sectors. This sub-industry universe has NOT been separately backtested — it's the already-failed rule set applied to a finer, untested universe, disclaimed accordingly. Collapsible section on the Sectors tab (Day 105)
...
### Known Open Issues (current, see `docs/claude/versioned/KNOWN_ISSUES_DAY107.md`)
...
- **v4.58: Scan Tab Verdict Column + Self-Description Correction** ✅ Fixed a real UI ambiguity: the Scan tab's breakout badge showed the same "—" dash for three different situations (beyond the 20-row check cap, requested but no result came back, and a genuine checked-and-neutral state) — each now gets its own honest label instead of one collapsed glyph (new Golden Rule 44). Built a new on-demand "STA Verdict" column, showing the real BUY/HOLD/AVOID verdict (reusing the existing decision pipeline, not a new one) only for rows already flagged Retest Entry or Building Base. Also corrected an inaccurate self-description carried since Day 1 — the app was described as "a Minervini system," but a full code audit confirmed it's a multi-engine workbench (Minervini/O'Neil trend-continuation alongside Connors mean-reversion, RRG sector rotation, Graham/Buffett/Lynch/Damodaran value investing, and Van Tharp risk sizing); updated the app's header tagline and this README's opening description accordingly (Day 106). **Day 107 follow-up (no version bump, research/backtest-only)**: built a plain-language "STA vs. 100 Years of Trading Principles" comparison (published as a Claude Artifact), code-audited against 3 independently-researched, cross-checked sources — found STA matches 7 of 9 classic trading principles, partially matches 1 (momentum lacks an absolute-return filter), and misses 1 (volume confirmation). A same-day backtest-only research spike tested both gaps directly: neither showed a credible edge (the volume-confirmation design tested was too strict to evaluate; the momentum fix tested clean but barely changed anything) — both stay parked, now with real data behind the "not yet." Zero live app code touched.
- **v4.60: MR Scanner + Volume Confirmation/Direction Shipped, Two Forward-Test Milestones** ✅ Wired an already-built, never-connected `/api/mr/scan` endpoint to a new "🔄 MR Signals" button on the Scan tab (existed since Day 81). Shipped a new Volume Confirmation + Direction read on the Trade Setup card in two Opus-planned phases: a magnitude read (today's volume vs. its 50-day average) and a directional lean (price change + close-location-in-range + OBV trend combined into an honest "leans buying/selling" or "mixed signals" read) — new `meta.candle` API field. A full technical audit graded Support & Resistance, MTF Confluence, and Pattern Detection's price target below industry standard against named methodology (S&R's "nearest support" pivot method was confirmed to actually select the most *extreme* level in range, not the nearest one); remediation plans written, not yet implemented. Two of the four live forward-test tracks crossed their 100-trade confirmation bar this session with opposite results under a cluster stress-test: mean-reversion (broad) genuinely confirmed (PF 3.0021, holds at PF 2.24 excluding its largest cluster); Momentum Path B clears the numeric bar (PF 1.2293) but fails the same test badly (66.7% of its sample entered in one 4-day window; PF 0.72 excluding it) — not treated as confirmed (Day 111).
- **v4.61: Path B Retired, S&R "Nearest Level" Fix, Measured-Move Pattern Targets** ✅ **Momentum Path B retired** — the real support/resistance-gate experiment reached 150 trades with no live edge (the only lift came from one 4-day regime cluster); it stops generating signals, its open positions wind down naturally, and its row stays as a historical record. **Support & Resistance fix (Golden Rule 53):** the pivot method's "nearest support/resistance" actually selected the most *extreme* levels in the lookback window, not the nearest — now fixed to genuinely pick the nearest levels, which also fixed a silently-broken confluence badge (~10% of levels) and restored per-level touch counts on the pivot path. A paired backtest showed the momentum config's profit factor was partly propped up by this bug (it inflated reward targets): corrected, Config C comes back at PF 0.53, and the old "PF 1.40" is formally retired. The fix ships anyway — it's a correctness win for the human-facing Analyze page, and the momentum config isn't a live track. **Pattern price targets:** Cup & Handle and Flat Base now project the actual measured move (cup depth / base height from the breakout pivot, O'Neil's rule) instead of a flat 20%/12%. Net: mean-reversion (broad) is now the only confirmed edge, and the momentum chapter is effectively closing (Day 112).
- **v4.62: Paper-Trading Discontinued; Volume/Momentum/MTF/VCP Informational Reads (consolidated, Days 113-118)** ✅ **Day 113: the entire automated paper-trading program was discontinued** at the user's explicit direction — STA is now a pure analysis/recommendation tool, the user places and manages all real trades themselves; no IBKR execution path, no new signals on any track, existing positions wound down naturally. **Day 116:** fixed a real OBV trend-classification bug (biased toward "rising" for negative cumulative OBV) and a separate, previously-unlogged bug where relative volume was computed from the still-forming intraday bar (silently understating volume conviction most of every trading day) — both fixed, plus a new same-day "effort vs. result" volume read. **Day 117:** Antonacci's dual-momentum "absolute momentum" check — previously invisible anywhere in the app — is now a visible, informational read on both the Simple Checklist and Full Analysis view, sourced from the project's own real backtest constant rather than an invented number; new Golden Rule 57 after catching that the obvious implementation would have silently added a 10th criterion to the 9-criterion checklist. **Day 118: completed the S&R fix chain (Golden Rule 53)** — MTF Confluence's confidence score, previously a hardcoded 1.0/0.6 flag nobody actually read, is now a real graduated Strong/Moderate/Weak score that no longer counts synthetic ATH/ATL levels against itself; the VCP pattern's price target, previously a flat and disclosed-as-fake +15%, is now the real nearest resistance level (or a measured contraction depth when none exists). Every change this stretch is purely informational — nothing here gates, ranks, or scores a trade decision.
...
### Current Priorities (Day 92-112 — forward-testing accumulation is the sole priority)
...
**Where the two trading systems stand (Day 112):** **mean-reversion (broad) is the sole confirmed edge** — 194 live paper trades, PF 2.56, 77% win rate, held steady across a week and survives every cluster stress-test since Day 93. **Momentum has failed every honest test:** the parallel "Path B" experiment (real support/resistance-based entry gate) reached 150 trades with no edge and was **retired** on Day 112; Path A (the frozen original) sits at PF 1.11 and drifting after 74 trades; and a corrected-support/resistance backtest of the momentum config came back at PF 0.53 (the old "PF 1.40" was partly an artifact of a level-selection bug that inflated reward targets — fixed Day 112). Path A is being left to finish its 100-trade bar, then the momentum chapter formally closes. The next real work is routing the confirmed MR (broad) track to real IBKR paper-trading execution.
...
**Day 110: forward-test check-in found a real clustering pattern, and the Regime redesign decision got reversed under direct pushback.** Built and published an interactive mockup of the Day 109 Analyze Page Redesign (`docs/claude/design/mockups/analyze_page_redesign_mockup.html`) so the design could actually be seen before any build decision. Pulled live forward-test numbers and ran a critical clustering check across all four tracks (the same discipline that already caught a bad pattern in the broad MR track back on Day 93) — found real single-day entry concentration in every track, most notably the curated HUB-65 track: 48.5% of its entire closed sample (16 of 33 trades) entered on one semiconductor-heavy day, and that cohort actually *underperformed* (43.8% win rate, roughly flat) while the rest of the sample ran far ahead (70.6% win rate) — the opposite direction from the usual "one lucky cluster inflates the number" pattern. Separately, the user directly challenged the Day 109 decision to keep Market Regime a hard gate on the Analyze page ("I will deal with regime and sector, why do you worry to put it in as a hard gate") — re-examined rather than defended, and reversed: Regime becomes informational like everything else on the page, kept expanded by default (not enforced) since it's portfolio-wide rather than single-stock. The live automated tracks' actual entry gate is untouched either way. New Golden Rule added after a real near-miss: a reply claimed a decision-doc edit had been made when the tool call hadn't actually happened yet — caught and fixed before this session closed, not after. No application code touched, no version bump.

**Day 111: two forward-test milestones with opposite outcomes, a deep technical audit, and two real features shipped — the longest session in the project's history.** A full technical audit graded five Analyze-page cards against named, real trading methodology: Price Structure and Breakout Status both scored well (9/10, matching published Dow Theory / institutional breakout-confirmation standards); Pattern Detection's pattern *recognition* also scored well (8/10, matches Minervini/O'Neil's published criteria) but its price *target* scored poorly (1.5/10 — a flat percentage, never checked against real resistance); MTF Confluence and Support & Resistance both scored below standard (3/10 and 2/10 — S&R's "nearest support" pivot method was confirmed via direct code read to actually select the most *extreme* level in range, not the nearest one). Remediation plans were written for all three, none implemented yet. A companion data-provenance map found the app actually runs on four separate data systems, not one pipeline, and fixed one real inconsistency (`/api/mr/scan` was bypassing the shared multi-provider orchestrator its sibling endpoint already used). Two real features shipped and were verified live: the MR Scanner button (wiring up an endpoint that had existed, unused, since Day 81) and a two-phase Volume Confirmation + Direction read on the Trade Setup card (see v4.60 above). Separately, two of the four live forward-test tracks crossed their 100-trade confirmation bar — mean-reversion (broad) genuinely confirmed under the same cluster stress-test that already caught real problems on Day 93/110 (138 closed, PF 3.0021, holds at PF 2.24 excluding its largest cluster — the first track in the project's history to clear this bar for real); Momentum Path B clears the same numeric bar (117 closed, PF 1.2293) but fails the identical stress-test — 66.7% of its entire sample entered in a single 4-day window (Aug 3-7), and excluding just those days flips the track to PF 0.72, net losing. Confirmed this is a new *regime/timing* correlation pattern, not the already-known sector-correlation gap. **Not treated as confirmed.** Before writing any code for the S&R fix, checked which live code actually calls the buggy function and found Momentum Path B's live entry gate imports it directly — fixing it in place would reset Path B's trade count under this project's own no-retuning-a-frozen-gate rule. That decision (fix in place and accept the reset / build a parallel corrected path / scope the fix to display only) is the single most important open item heading into the next session — Path B's own stress-test failure this session makes "accept the reset" look materially less costly than it did before, but no decision has been made yet.

**Day 109: Analyze Page Redesign — design phase complete, no code changed.** Investigated whether the Analyze page's decision architecture (which signals gate a trade vs. inform a human) should change, given 9 months of real usage. Worked through it as an item-by-item back-and-forth rather than one proposed plan — each of 17 components (Technical structure, Market Regime, Fundamentals, Risk:Reward, position sizing, holding period, and more) got its own research pass and discussion before being locked. Net direction: Technical structure and Market Regime stay mechanical (Regime actually gains a new, more-visible gate, not less); Fundamentals becomes an evidence-based binary red-flag screen instead of a scored verdict input; Risk:Reward and position sizing become fully informational. A dedicated deep-validation pass caught 4 real corrections in the initial reasoning (an already-closed Known Issue nearly got re-opened; a claimed UX fix turned out to be dead code; a mechanism change would have silently dropped ADX from the redesigned read) and found a real, previously-undiscovered bug along the way: the Momentum entry panel's position-size label has shown a hardcoded "half" with zero computation behind it since Day 39. Full decision record: `docs/claude/design/ANALYZE_PAGE_REDESIGN_DECISIONS.md`. **Not implemented** — the 4 live forward-test tracks and their backend logic are explicitly untouched and confirmed to stay that way; this is design-only, build deferred until requested.

1. **Decide the S&R fix scope** — `_pivot_sr()`'s confirmed extreme-vs-nearest bug sits directly underneath Momentum Path B's live entry gate; fixing it in place resets Path B's trade count. Three options on the table (fix in place / build a parallel corrected path / scope to display-only), not yet decided (Day 111).
2. **Let paper trading accumulate — SOLE FOCUS**, now across four tracks (Path A, Path B, broad MR, HUB-65 MR). As of Day 111: mean-reversion (broad) is genuinely **confirmed** (138 closed, PF 3.0021, holds under a cluster stress-test); Momentum Path B crosses the numeric bar but fails that same stress-test (117 closed, PF 1.2293 headline, PF 0.72 excluding one 4-day cluster) — not treated as confirmed. Check progress in the Forward Test tab's status panel, or via `daily_job.py --report`.
3. *(parked)* **Fundamentals mitigation decision** — a measured 40% disagreement between live and backtested fundamentals data sources is still pending a choice (align live-to-SimFin or backtest-to-TTM).
4. *(parked)* **Confirm SimFin key rotation** — small housekeeping item.
5. *(parked)* **N3 gap-fill detection** — needs its own design session first (no spec exists yet).
6. *(parked)* **Value Tab Phase 2** — needs its own batch-prefetch infrastructure design session first (AlphaVantage free-tier budget constraints).
7. *(parked)* **Volume confirmation missing from the decision engine** — ~~neither the Full Analysis verdict nor the Simple Checklist check whether a price move is backed by rising volume~~ **DONE (Day 111)** — shipped as an Info-only Volume Confirmation + Direction read on the Trade Setup card, not a gate (see v4.60 above).
8. *(parked, idea only)* **Scheduled/proactive breakout-alert watcher** — the breakout engine already works on demand against any ticker list (`/breakout-watch`, Scan tab badges); nothing today runs on a schedule or notifies unprompted. Flagged Day 100, needs its own design session before any build.
9. *(parked)* `/ibkr-scan` skill, Price Structure Phase 3 (visual chart) — queued behind the above.
10. **Dual/absolute momentum filter** — ~~parked, low priority~~ **surfaced as information, Day 117.** Antonacci's dual-momentum research (found Day 107) says a stock should also beat a risk-free/cash return in absolute terms, not just beat the market relatively. The *gate* decision stays closed exactly as Day 111 left it — Config G backtested clean but only excluded 1 of 75 trades, correct but pointless — but the read itself is now shown to the user: both the Simple Checklist's Momentum card and the Full Analysis RS card show the stock's own trailing 1-year return against a 5% cash proxy, informational only, never gating. Zero backend changes; reuses a number both views already computed. See `docs/claude/design/ABSOLUTE_MOMENTUM_READ_PLAN_DAY117.md`.
11. *(mockup built Day 110, not built)* **Analyze Page Redesign** — full decision record, an interactive mockup (`docs/claude/design/mockups/analyze_page_redesign_mockup.html`), and a phased implementation plan at `docs/claude/design/ANALYZE_PAGE_REDESIGN_DECISIONS.md`; includes a real, unfixed bug (a hardcoded fake "half" position-size label). Day 110: the Market Regime decision reversed from a hard gate to informational; the mockup itself still shows the pre-reversal copy, not yet updated. Build only when explicitly raised — freeze-independent, but still behind Priority #1.
12. *(parked)* **Day 108 full-audit findings needing a decision or bigger scope** — VIX-based position sizing was never wired into the live automated engine (verified this doesn't affect any current live-track statistic, so not urgent); a couple of UI wording questions on the Value tab. Full detail in the project's internal audit log.
13. ~~*(parked, Day 111)*~~ **MTF Confluence's arbitrary confidence multiplier, and Pattern Detection's flat-percentage price target** — **DONE (Day 118).** Both graded below industry standard in a Day 111 technical audit (3/10 and 1.5/10); Cup & Handle/Flat Base fixed Day 112. MTF Confluence's `strength` is now a real graduated score (Strong/Moderate/Weak) that excludes synthetic ATH/ATL-projected levels from the confluence set; VCP's target is now real nearest-resistance (falling back to measured contraction depth) instead of a flat +15%. See `docs/claude/design/MTF_VCP_FIX_PLAN_DAY115.md`.
...
**Day 103: SRPS investigated end-to-end as a potential 5th forward-test track — failed its own backtest, pivoted to a discretionary screener.** A fully-specified design (buy strong stocks in recovering sectors as they pull back to their 21-EMA) was run through its own two pre-registered gates before any live build: Gate 0 (signal frequency, a new historical sector-quadrant replay tool) passed comfortably; Gate 1 (a new day-by-day portfolio backtest — 400 tickers, survivorship-free, 2020-2025, with a real max-6-concurrent-position cap, the first backtest in this project built that way) failed cleanly (34.1% win rate vs. required 45%, PF 1.177 vs. required 1.2). Three real bugs were found and fixed along the way — a rule-interaction whipsaw (an entry condition that could trigger its own exit condition almost immediately), a universe-construction step that silently excluded real historical bankruptcies before a survivorship-free test could count them, and a near-zero stop-distance bug that produced a nonsense 331,408 R-multiple on one trade. Per the design's own rule, SRPS does not get built as a live automated track — the same mechanical logic was repurposed instead into a discretionary Sectors-tab screener (candidates only, permanent disclaimer, user applies own judgment). Also adopted the Trading Intelligence Hub's escalating three-pass code review as new Golden Rule 41, and fixed a real, user-reported Forward Test tab bug (the closed-trades table silently capped at 20 rows for the two higher-volume tracks with no indication anything was hidden). Freeze-independent throughout — none of the 4 live forward-test tracks were touched.
...
*Last Updated: August 28, 2026 (Day 111)*
