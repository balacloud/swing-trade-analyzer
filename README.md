...
13. *(parked, Day 111)* **MTF Confluence's arbitrary confidence multiplier, and Pattern Detection's flat-percentage price target** — both graded below industry standard in a Day 111 technical audit (3/10 and 1.5/10). Remediation plans written, not implemented. MTF Confluence's fix depends on the S&R decision above (Priority #1) since confluence is only as good as the levels it confirms; Pattern Detection's Cup & Handle / Flat Base fix has no such dependency and can ship independently.
