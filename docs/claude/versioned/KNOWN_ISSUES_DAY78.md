...
### Low: Backtest↔Live Fundamentals Data-Source Mismatch (Day 78 — Fable review)
**Severity:** Low (unmeasured; may be Medium after diagnostic)
**Description:** Backtest ROE = SimFin quarterly ×4; live ROE = Finnhub TTM. Same stock can get different Fundamental labels → live Config C ≠ backtested Config C.
**Fix:** Remediation Task 3.2 — measure label disagreement rate on ~20 tickers; escalate if >20%.
