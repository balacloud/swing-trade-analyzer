...
> **Version:** Day 70 (March 19, 2026)
> **Total API Routes:** 28 (verify with `grep -n "@app.route" backend.py`)
>
> **Day 70 Changes:** Added 2 mean-reversion endpoints (`/api/mr/signal/<ticker>`, `/api/mr/scan`). Frontend v4.31, Backend v2.33, API Service v2.10.
...
### Mean-Reversion (Day 70 — Tier 3A)
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/mr/signal/<ticker>` | GET | RSI(2) oversold signal for single ticker. Returns rsi2, sma200, conditions, stop/target. |
| `/api/mr/scan` | GET | Scan universe for MR signals. Optional `?tickers=AAPL,AMD,...`. Returns list of active signals sorted by RSI(2) ascending. |
...
### GET `/api/mr/signal/<ticker>` (Day 70 — Tier 3A)

**Purpose:** Check if a ticker has an active mean-reversion signal (RSI(2) < 10 + above 200 SMA).
...
### GET `/api/mr/scan` (Day 70 — Tier 3A)

**Purpose:** Scan universe for mean-reversion signals. Returns tickers where RSI(2) < 10 AND price > 200 SMA.

**Query params:** `?tickers=AAPL,AMD,NVDA` (optional — defaults to ~50 liquid stocks)

**Response:**
```json
{
  "signals": [
    { "ticker": "INTC", "signal": true, "rsi2": 6.42, "sma200": 28.50, ... }
  ],
  "scanned": 50,
  "found": 1
}
...
**Notes:**
- Signals sorted by RSI(2) ascending (most oversold first)
- RSI(2) < 10 is a rare event — most scans return 0-2 signals
- Uses yfinance for data (1y history per ticker)

---

*This file is versioned by day. Current version: DAY70*
*Previous version: API_CONTRACTS_DAY62.md (updated Day 68)*
