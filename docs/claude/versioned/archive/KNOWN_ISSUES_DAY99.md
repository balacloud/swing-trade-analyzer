# Known Issues — Day 99 (July 27, 2026)

## Changes from Day 98

**Resolved this session:** the partial-bar ledger contamination bug — a HIGH-severity data-integrity issue where mid-session manual runs (`--force`/Force Run Now) could write an intraday, not-yet-final price into the ledger as a closing price and permanently close a position. Fixed in 4 phases (guard against the incomplete bar, one-time repair of 20 affected rows, UI warning, two minor cleanups) — see `docs/claude/design/PARTIAL_BAR_LEDGER_CONTAMINATION_FIX_PLAN.md` for the full diagnosis and `PROJECT_STATUS_DAY99_SHORT.md` for what shipped. All 27 remaining closed trades now re-replay exactly against complete history — verified exhaustively, not sampled.

**Added this session:** none new — the bug above was found, diagnosed, and fixed within the same session, so it never carried forward as an open item.

**Freeze status:** unchanged — forward-testing accumulation remains the sole stated priority across all four tracks. The IBKR plan (Day 97) is still explicitly parked. A "Quick" momentum holding-period idea was discussed and planned but the user stopped before approval — not built, not tracked as an open item (see Next Session Priorities in the status doc if picked up later).
...
**Severity:** Low / Info
**Description:** `backtest_hub_mr.py`'s clean-looking headline numbers (PF 1.2574, Sharpe 1.5278) are NOT comparable to the project's own survivorship-free baseline (PF 1.16). The caveat is stated everywhere the number appears — this entry exists so it isn't quietly forgotten, not because it's hidden anywhere.
...
**Fix:** Not actionable — accepted by design. Frame any future comparison as "a different bet," not "a better strategy."
...
**Severity:** Low
**Description:** Value tab's ROE thresholds badged "Buffett/Damodaran" when the code comment says "ChatGPT research validated"; Validate/Data Sources tabs show "live"/"healthy" status without probing real fetch success; Forward Testing's momentum-path trades store identical net/gross P&L (**note: partially addressed this session** — momentum's live close path now nets transaction costs, Phase 4 of today's fix — but the display-layer part of this finding, if any, is untouched) and per-position fetch failures are silently dropped.
