...
## Current Version: v4.50 (Backend v2.44, Frontend v4.45, Backtest v4.19, API Service v2.11)
*Day 93: v4.50 — Sectors/Context tab audit (independent of the paper-trading freeze — pure display/UI logic, zero contact with the verdict engine): 3 real bugs fixed on the Sectors tab (mid-cap-blind banner text, bar-color-vs-label contradiction, plus a full beginner-focused redesign of the CTA/card layout after user review), 2 real bugs fixed on the Context tab (a Day 91 regression in the econ composite's PMI lookup, a Seasonal Regime text/badge contradiction), and a new cross-tab connection (`macro_alignment` on `/api/sectors/rotation`, plus a Market-Phase-vs-Macro-Regime reconciliation on the Context tab itself). A self-audit against `GOLDEN_RULES.md` on this session's own work found and fixed a real DRY violation (2 new Golden Rules 30-31 added). See `KNOWN_ISSUES_DAY93.md` and `API_CONTRACTS_DAY93.md`. Also corrected a real version-drift: `BACKEND_VERSION` in code was still '2.43' despite docs claiming v2.45 as of Day 92 — code is ground truth, bumped from there.*
