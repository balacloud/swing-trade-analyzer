...
## Current Version: v4.62 (Backend v2.51, Frontend v4.59, Backtest v4.21, API Service v2.11)
*Days 113-118 (consolidated close — no formal `/sta-end` ran between Day 112 and this one):
**Day 113:** paper-trading program DISCONTINUED at the user's explicit direction ("I don't want
to paper trade... I want to use Claude and STA as analysis engine and recommendation, I will
take care of stuff myself") — no new signals on any track, existing positions wind down
naturally, MR (broad) stands as the confirmed result (194+ closed, PF ~2.5). **Day 116:**
`calculate_obv()`'s `trend` sign bug fixed (was biased toward "rising" for negative cumulative
OBV); its 20-day divergence sentence promoted from hover-only to visible text; a real, unlogged
`meta.rvol` partial-bar bug found and fixed same session (was silently understating volume
conviction most of every trading day); a new same-day effort-vs-result read added. **Day 117:**
Antonacci's dual-momentum "absolute momentum" leg — previously invisible entirely — surfaced as
an informational read on both the Simple Checklist and Full Analysis view, sourced from the
real Day 107 backtest constant. New Golden Rule 57 after the Opus plan caught that the natural
implementation would have silently added a 10th Simple Checklist criterion. **Day 118:**
Phases 3-4 of the S&R fix chain shipped — MTF Confluence's dead `strength` field is now a real
graduated score with synthetic-level exclusion; VCP's flat +15% target replaced with real
nearest-resistance / measured-contraction-depth. All four days' work is pure Analyze-page
display/read logic, freeze-independent, zero gates touched (none exist anymore post-Day-113).
Full detail: `docs/claude/status/PROJECT_STATUS_DAY118_SHORT.md`,
`docs/claude/versioned/KNOWN_ISSUES_DAY118.md`, `API_CONTRACTS_DAY118.md`.*
*Day 112: v4.60→v4.61 (Backend v2.48→v2.49, Frontend v4.55→v4.56 — code version constants were drifted at v2.47/v4.54, corrected). Three code changes, all verified live. **(1) Momentum Path B RETIRED** — the real-S&R-gate momentum experiment reached 150 closed trades with no live edge (blended PF ~1.01; the only lift was one Aug 3-7 regime cluster, ex-cluster PF 0.72). `live_signals.py` no longer generates Path B signals; open/pending positions wind down by their own rules; `PAPER_TRADING_PREREGISTRATION.md` §8b marked RETIRED. Not deleted — the row stays as a historical record. **(2) Pattern price targets** — Cup & Handle / Flat Base now use measured moves (cup depth / base height projected from the pivot, O'Neil's rule) instead of a flat 20%/12%; returns null rather than a fabricated % when geometry is missing; VCP keeps a flagged flat +15% pending Phase 4. **(3) S&R `_pivot_sr` nearest-level fix (Golden Rule 53)** — now selects the N pivots nearest to price per side (merge-not-reject on min spacing), behind `SRConfig.pivot_nearest_selection`; touch scoring added to the pivot path; two new bugs fixed in the same pass — BUG-A (a key-format mismatch that silently killed the ★ confluence badge for ~10% of levels) and BUG-B (Price Structure showed the *deepest* supports, not the nearest — GR53 recurring in the frontend). Verified live: META R1 now $612.43 (0.3% above) vs. the pre-fix extreme ~$745. **Config C backtest re-baselined**: paired A/B on the frozen universe showed PF 0.97 → 0.53 with the corrected levels (the pre-fix number was partly a bug artifact — R:R inflated by a far resistance target). The pre-registered directional prediction ("trades rise") was wrong and logged as such (new Golden Rule 56). Per Golden Rule 20: run once, accept it — no re-tuning. Day 79's "PF 1.40" is formally retired (already un-reproducible since Day 107 per GR45). This also closes the "corrected-S&R momentum forward-test variant" question — the pre-committed gate was "backtests better → pre-register a variant"; it backtested worse. New Golden Rule 55 (scoped Opus-plans-then-Sonnet-implements). New `--momentum-only` backtest flag; additive `meta.pivotSelection` API field. Full detail: `PROJECT_STATUS_DAY112_SHORT.md`, `KNOWN_ISSUES_DAY112.md`, `SR_PIVOT_FIX_PLAN_DAY112.md`, `SR_PIVOT_FIX_REBASELINE_DAY112.md`, `API_CONTRACTS_DAY112.md`.*
...
|--------|--------|----------|----------------|--------|--------------|
| **Config C (momentum)** — Day 79, ~~canonical~~ **RETIRED** | 114 | 49.12% | ~~**1.40**~~ (was 1.61) | 0.52 (was 0.85) | No — block bootstrap p=0.094 |
| **Config C (momentum)** — Day 107 re-run (SimFin drift, GR45) | — | — | ~0.97 | — | not reproducible from seed 42 anymore |
| **Config C (momentum)** — **Day 112, current canonical** (corrected S&R, `_pivot_sr` nearest-selection) | 41 | 36.59% | **0.53** | −0.98 | No — p=0.938, net losing |
...
**Day 112 re-baseline (Config C):** the "PF 1.40" was already un-reproducible since
Day 107 (Golden Rule 45, SimFin universe drift). Day 112's S&R `_pivot_sr` fix
(nearest-level selection, Golden Rule 53) revealed it was *also* partly a bug
artifact — the extreme-level bug set the R:R reward target to the highest high in
2 years, inflating R:R past the 1.2 gate on trades that were never real 1.2:1
setups. Paired A/B on the frozen seed-42 universe: **PF 0.97 → 0.53**, trades
75 → 41, corrected. This is the honest number; per Golden Rule 20 it stands with
no re-tuning. The live forward test already said the same thing — Momentum Path B
(real S&R gate) was retired the same session as a null. Full detail:
`docs/claude/versioned/SR_PIVOT_FIX_REBASELINE_DAY112.md`.
...
## ⛔ Day 113 (2026-09-12): PAPER-TRADING PROGRAM DISCONTINUED — FREEZE LIFTED

At the user's explicit direction, the entire automated paper-trading /
forward-testing program is discontinued: **"I don't want to paper trade...
I want to use Claude and STA as analysis engine and recommendation, I will
take care of stuff [myself]."** STA is now used purely as an on-demand
analysis/recommendation tool — the user places and manages real trades
themselves. `daily_job.py` no longer generates any new signal on any track
(`PROGRAM_DISCONTINUED` flag); every open/pending position winds down by its
own exit rules; `launchd` stays loaded only until every track shows 0
open/0 pending, then gets unloaded. **MR (broad) closes the program out as
its one genuinely confirmed edge** (194+ closed, PF ~2.5, survived every
cluster stress-test since Day 93) — full detail in
`docs/claude/stable/PAPER_TRADING_PREREGISTRATION.md`'s discontinuation
notice and Change Log row 113.

**This lifts the Day-92 "forward-testing accumulation is the sole priority"
freeze that governed every item in the table below.** IBKR real paper
execution (Priority #13) is closed as **declined**, not parked. Every other
parked item is fair game again whenever the user wants it — re-prioritize
deliberately rather than treating "the freeze is over" as "do everything at
once."

## ACTIVE PRIORITY ORDER (Day 92 updated — forward-testing accumulation is the sole priority, all others parked) — **HISTORICAL, superseded by the Day 113 notice above**

| # | Item | Why | Effort |
|---|------|-----|--------|
| 1 | **Let paper trading accumulate** | SOLE FOCUS (Day 92 — user explicitly holds all other priorities below this until the bar clears) — automated engine (`backend/paper_trading/`) built and live Day 81, running unattended daily via launchd. Both momentum (PF 1.40) and MR (PF 1.16, post liquidity re-test) still need **100 live trades each** before capital allocation (raised from 50, Day 92 — see `PAPER_TRADING_PREREGISTRATION.md` Change Log). As of Day 92: momentum 0 closed, MR 5 closed — both far from either bar. Check in via the Forward Test tab's status panel (now with an expandable per-ticker table, Day 92) or `daily_job.py --report`. **Day 111 update: first two tracks crossed the bar, with sharply different results under scrutiny.** MR (broad): 138 closed, 79.71% WR, PF 3.0021 — genuinely confirmed, stress-tested via cluster removal (PF held at 2.24 excluding its largest 3-day cluster, WR *rose* to 80%). Momentum Path B: 117 closed, PF 1.2293 — clears the numeric bar on paper, but 66.7% of its entire sample (78 of 117 trades) entered in one 4-day window (Aug 3-7); excluding just those 4 days leaves PF 0.72 (net losing). **Not being treated as a clean confirmation.** **Day 112 update:** Path B **RETIRED** at 150 closed (no live edge, ex-cluster PF 0.72) — the real-S&R momentum gate has no edge, and the Day 112 corrected-S&R Config C backtest (PF 0.53) agrees. **MR (broad) is now the sole confirmed edge** — 194 closed / PF 2.56 / WR 77% as of 2026-09-09, held steady across a week. Momentum Path A (74/100, PF 1.11 and drifting) should be **let to finish its bar and then formally closed** — the momentum chapter is effectively over. Priority shifts down-list to #13 (IBKR real paper execution for MR broad — the actual next work). **Day 113: the whole program is DISCONTINUED at the user's explicit direction** — see the notice above this table. `daily_job.py` generates no new signals on any track; existing positions wind down naturally; MR (broad) stands as the confirmed result. | **DISCONTINUED (Day 113)** |
...
| 13 | **⛔ DECLINED (Day 113) — not parked, closed.** User explicitly decided against any automated execution path: "I don't want to paper trade... I will take care of stuff myself." STA stays an analysis/recommendation tool; the user places and manages real trades. Design doc kept for reference only. ~~IBKR real paper-trading execution — THE NEXT REAL WORK (Day 112).~~ Full phased design doc: `docs/claude/design/IBKR_PAPER_EXECUTION_PLAN.md` (Day 96). **Day 112 re-scope:** the design doc targets "Path B's momentum signals" — Path B is now retired and momentum has failed every honesty test (Path B null, Path A PF 1.11 drifting, Config C corrected backtest PF 0.53). The track worth deploying is **MR (broad)** — 194 closed / PF 2.56 / WR 77%, the sole confirmed edge, held steady across a week. The plan's mechanics (gateway/auth/order endpoints, the `DU`-prefix account assertion, the 4 phases) transfer directly; only the signal source changes. Still parked on the user's own CIRO Rule 3200 confirmation with IBKR support (the design doc's "Open items"). Goal: execute MR (broad) signals as real bracket orders against the user's IBKR paper account (Client Portal REST API), so fills come from IBKR's own paper engine — realistic slippage against live market data — instead of the internal simulator. Feasibility (gateway setup, auth flow, order-placement endpoints) and the CIRO Rule 3200 regulatory question (researched directly against CIRO's own published guidance — governs real orders to a real marketplace, doesn't appear to have anything to regulate in a pure paper account) are both resolved in the design doc. Hard constraint locked in: every order-placing code path must assert the target account ID starts with `DU` (paper-account prefix) before calling any order endpoint, no bypass. 4-phase plan (connectivity+one manual test order → wire into real signals, still manual → ledger/reporting integration → operational hardening, honest about the no-unattended-2FA gap). **Not started** — user wants to do more research/review before Phase 1 begins; the design doc's own "Open items" section notes confirming the CIRO paper-account question directly with IBKR support as the one loose end worth closing first. | Design done; 4-phase build once resumed |
