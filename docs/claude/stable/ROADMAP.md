...
## Current Version: v4.59 (Backend v2.47, Frontend v4.54, Backtest v4.20→v4.21, API Service v2.11)
...
| 11 | **Volume-confirmation not in the core verdict — first design tested Day 107, needs a re-design before another backtest** | Found in a post-Day-91 first-principles review of the decision engine. Neither the Full Analysis verdict tree nor the Simple Checklist's 9 criteria check whether a move is confirmed by rising volume vs. thin volume — the Simple Checklist's "Volume" criterion is a **liquidity gate**, not a price/volume-confirmation signal. **Day 107 update**: a backtest-only research spike (`research_spike_volume_momentum.py`, Config F — entry-day volume ≥1.5x 50-day avg, layered on Config C) tested this directly on the 400-ticker survivorship-free universe. Result: cut trades from 75 to 5 — too strict to evaluate, a structural finding not a statistical one. **Not deployable as tested**; needs a genuinely different filter shape (lower ratio, or a secondary quality signal instead of a hard gate) before another backtest is worth running. Still explicitly gated behind the paper-trading 100-trade bar (Priority #1) regardless of design — see `KNOWN_ISSUES_DAY107.md`. Companion item, also spiked Day 107: absolute/dual momentum (Antonacci) — `Config G` (stock's own trailing 12mo return > 5% risk-free, on top of the existing RS≥1.0 relative check) tested clean (74 real trades) but only excluded 1 of Config C's 75 trades — technically sound, near-inert in practice, low priority, cheap to add once the freeze lifts. `mean_reversion.py`'s ADX docstring/code mismatch (carried, unchanged) — likely a doc-accuracy fix, resolve alongside the volume work. | Design (volume, needs a real rethink) + re-backtest, post-freeze; trivial add (momentum) or doc fix (ADX), post-freeze |
...
| 17 | **Analyze Page Redesign — design phase complete (Day 109), implementation not started.** Full decision record at `docs/claude/design/ANALYZE_PAGE_REDESIGN_DECISIONS.md`: 17 locked decisions on which Analyze-page signals stay mechanical (Technical structure, Market Regime — the latter gains a new always-visible gate) vs. become informational (Fundamentals as a red-flag screen, Risk:Reward, position sizing), plus a default-view change (Simple→Full) and a full phased implementation plan with a freeze-safety audit confirming zero impact on `backend/support_resistance.py`, `categorical_engine.py`, or any of the 4 live forward-test tracks. A real bug was found and documented (not yet fixed): the Momentum entry panel's "half" position-size label has been hardcoded with zero computation behind it since Day 39. **Not started** — build only when explicitly requested; this is freeze-independent display-layer work, same category as Days 93/101/103/105/106/108, but still gated behind Priority #1 like everything else on this list. | Design done; 6-phase build once resumed |
