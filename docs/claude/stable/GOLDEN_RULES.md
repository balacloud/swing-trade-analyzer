...
26. **DON'T ACCEPT AN EXTERNAL AUDIT'S DIAGNOSED ROOT CAUSE WITHOUT REPRODUCING IT DIRECTLY — A PLAUSIBLE GUESS CAN BE WRONG IN A WAY THAT MAKES THE SUGGESTED FIX A NO-OP.** A hub-side audit (Session 28, `HANDOFF_sta_audit_session28.md`) found the Context tab's CPI card showing stale-looking values (3.7%/2.8% vs. the real 3.5%/2.6% BLS release) and guessed the cause was cache staleness, suggesting a shorter TTL. A direct live check told a different story: hitting `/api/econ` fresh (`from_cache: false`) reproduced the exact same "wrong" number — proving the cache TTL (already 6h) was never the problem. Tracing the raw FRED response found the real cause: FRED had withheld one month's observation (`2025-10-01: "."`), and `_yoy()`'s fixed-list-index lookback (`series[periods]`) silently shifted onto the wrong calendar month once that gap was filtered out — a date-alignment bug with nothing to do with caching. Shortening the TTL (the audit's suggested fix) would have changed nothing. Before implementing a suggested fix — from an audit, a teammate, or your own first guess — reproduce the symptom yourself against live/fresh data and confirm the named cause actually explains it; only then is the fix guaranteed to do something. (Day 91, Context tab CPI/PMI fix)
28. **STAMP DATES FROM THE DATA, NOT THE WALL CLOCK — A FORCE-RUN ON A NON-TRADING DAY CAN PERMANENTLY ZOMBIE A SIGNAL.** `paper_trading/daily_job.py` computes `today = datetime.now().strftime('%Y-%m-%d')` and `live_signals.py` stamped every new signal's `signal_date` with that same raw wall-clock value, even though `signal_price` was correctly read from the ticker's actual last OHLCV bar two lines above it. Force-running the job on a weekend (the "Force Run Now" UI button has no day-of-week guard) stamped `signal_date` as a Sunday — a date that can never appear in any OHLCV DataFrame's trading-day index. `activate_pending_signals()`'s `_find_index_for_date()` lookup then failed on that row forever, every single day, with only a `print()` nobody sees (the UI trigger route is synchronous and aggregate-only, per the Day 90 finding) — a silently, permanently stuck `pending_entry` that would never become a real trade. Found by re-running the job in the foreground after a Force Run "looked like nothing happened" and reading the actual stdout. 8 of momentum's 11 pending signals were affected — a meaningful fraction of an already slow-accumulating sample. Fixed by deriving `signal_date` from `stock_df.index[-1]` (the same row `signal_price` already came from) instead of `datetime.now()` — self-consistent by construction, no market-calendar library needed. The 8 already-stuck rows were repaired by matching their stored `signal_price` back to the OHLCV row it actually came from (unique price match within the recent window) and correcting `signal_date` to that row's real date; all but one activated on the next run (the last was correctly still waiting on a stale cache, not stuck). Same family as Rule 27: don't trust "when I ran this" as a proxy for "what the data says happened" — read the data's own timestamp. (Day 92, force-run investigation)

29. **A LIVE PAPER-TRADING REPLAY MUST READ BACK VALUES ALREADY LOCKED IN AT ENTRY, NOT RECOMPUTE THEM FRESH EVERY DAY.** `daily_job.py`'s `step_open_positions()` called `simulate_trade()`/`simulate_mr_trade()` without passing the `initial_stop_price`/`initial_target_price` the ledger had already stored at entry (`compute_entry_levels()` already existed for exactly this purpose) — so every daily replay silently recomputed stop/target fresh from whatever the current OHLCV window and code produced. This isn't just a hypothetical: verifying the fix caught a live instance of the exact drift risk it prevents — KRYS's freshly-recomputed stop (332.18) had already drifted from its stored entry value (333.37) due to an upstream provider data revision, with zero trace that a drift had occurred. A future edit to the entry-level formula would have the same silent effect on every already-open position. Fix: pass the ledger's stored `initial_stop_price`/`initial_target_price`/`max_hold_days` as overrides into the simulator (the override params already existed, just weren't wired up) — anchor every replay to what was actually decided at entry, not to what today's code/data would decide if asked fresh. Same integrity principle as Rule 21 (DRY the exit logic) one layer deeper: sharing the *function* isn't enough if the *inputs* to that function are allowed to drift. (Day 91, paper-trading exit-rule integrity fix, Session 28 audit)
...
33. **A SCHEDULED JOB'S "LOCAL TIME" COMMENT IS A CLAIM ABOUT THE MACHINE, NOT A FACT — VERIFY IT.**
 The paper-trading `launchd` plist's `StartCalendarInterval` fired at Hour=16 with a comment claiming "16:30 Central Time (this machine's local timezone)." A user question ("is it CST or EST") prompted checking `/etc/localtime` directly, which showed the machine's real local timezone is America/Toronto (Eastern) — `launchd` interprets Hour/Minute fields in whatever the OS actually thinks is local time, not whatever a comment assumes. The mismatch silently shrank the intended ~90-minute post-market-close settling buffer (chosen so daily bars had time to settle across the TwelveData/yfinance/Stooq provider chain) down to only ~30 minutes, for an unknown number of sessions, with zero symptom loud enough to surface it — the job kept running and recording trades, just with less buffer than designed. Any time a scheduled job's documented "fires at X in local time" matters for a downstream assumption (data settling, market-hours alignment, rate-limit avoidance), check the actual system timezone (`date`, `/etc/localtime`, `systemsetup -gettimezone`) rather than trusting whatever a prior comment or setup script assumed — the machine may have been reconfigured, or the assumption may simply have been wrong from the start. (Day 95, paper-trading schedule fix)

34. **APPLY THE MASTER-TRADER PERSONA LENS TO JUDGMENT CALLS, NOT JUST CODE CORRECTNESS.** `docs/claude/stable/PERSONA.md` defines a 30-year, all-cycle veteran trader's decision-making lens — first-principles discipline, a checklist of classic market "don'ts," and named behavioral-finance pitfalls each grounded in a real moment this project has already been caught by (recency bias on MR's clustered 95.65% WR, anchoring on the R:R gate's flat +8% target, survivorship bias on the original 60-ticker backtest, and others). GOLDEN_RULES.md governs how Claude *works*; PERSONA.md governs how Claude should *think* when evaluating a threshold, a backtest/live result, or a "can we speed this up" request. Load it at session start alongside this file; log anything it catches or confirms in its own Feedback Log at session close, the same accumulation pattern this file already uses. Built directly from a real Day 95 conversation (the "speed to 100 trades" request) where this reasoning was applied informally before the file existed — the point of writing it down is to stop re-deriving it from scratch every session. (Day 95, user-requested)

35. **LIVE/BACKTEST LOGIC PARITY MUST BE VERIFIED DIRECTLY, NOT ASSUMED FROM A SHARED NAME OR COMMENT.** `paper_trading/live_signals.py`'s momentum R:R check has called `backtest/trade_simulator.py`'s `compute_entry_levels()` (flat +8% target, ATR-clamped stop) since Day 81, and its own docstring even claimed this used "the exact same thresholds the backtest uses." It didn't. `backtest_holistic.py`'s actual Config C entry gate (`check_entry_signals()`) computes R:R from real support/resistance levels entirely — `compute_entry_levels()` was only ever the *exit-management* formula in the real backtest, never the entry decision. This went undetected from Day 81 (when the automated engine was built) until Day 95, when a live investigation into a suspiciously high rejection rate led to actually diffing the two code paths against each other — nobody had had a reason to compare them before. Same bug class as Golden Rule 19 (JS/Python verdict parity), one layer over: whenever a live/production path is built to "replay" or "match" a backtest's validated logic, trace the actual call graph and confirm it calls the *same function* the backtest's real decision point uses — a plausible-sounding docstring claim is not verification, and two functions computing "R:R" is not evidence they compute the *same* R:R. Fixed by building Path B, a parallel experiment using the real S&R-based gate, tracked separately so Path A's existing count stays valid. (Day 95, R:R gate investigation)

36. **A CIRCUIT BREAKER MUST DISTINGUISH "THIS TICKER HAS NO DATA" FROM "THIS PROVIDER IS UNHEALTHY" — THEY ARE NOT THE SAME FAILURE.** Every one of this project's 6 data providers called `breaker.record_failure()` immediately before raising `DataNotFoundError`/`InsufficientDataError` — the same failure tally used to decide whether to trip the circuit breaker and block all further requests to that provider. A ticker genuinely having no data on a given provider (BRK.A on Tradier, a delisted symbol, an unusual dual-class ticker) says nothing about whether the *provider* is healthy — but 3 such misses in a row (a plausible coincidence during any broad daily scan) tripped the breaker exactly as if the provider were down, blocking every *other*, perfectly-servable ticker in that run for a flat 5-minute cooldown. Found investigating why Tradier "didn't help" during a forward-test run: a direct re-test proved it fully healthy (10/10 known-liquid symbols, all <0.5s) immediately after the tripped cooldown window, confirming the earlier trip was a false signal, not real unhealthiness. Fixed across all 6 providers: only genuine connectivity/timeout/rate-limit/auth failures (`ProviderUnavailableError`, `RateLimitError`, `AuthenticationError`, or an uncaught exception) count toward the breaker; a ticker-specific `DataNotFoundError`/`InsufficientDataError` no longer does. When building any health-tracking mechanism (circuit breaker, retry budget, alerting threshold), separate "this specific request's target doesn't exist/qualify" from "the thing serving requests is broken" — conflating them makes the mechanism fire on noise instead of the signal it was built to catch. (Day 95, provider circuit-breaker fix)
