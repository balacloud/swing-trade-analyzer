...
**RETIRED — Day 112.** Path B ran from Day 95 to Day 112: **150 closed / 67 open / 15 pending** at retirement. Conclusion: the real S&R-based entry gate showed **no live edge** over 150 closed trades — blended PF 1.0145, expectancy +0.04%/trade, avg R −0.003. At its own 100-trade milestone (2026-08-25) it briefly cleared the Section 10 "Confirmed" bar (PF 1.37, +1.03%/trade), but **69 of those first 100 closed trades entered a single 4-day window (Aug 3–7 2026)**; ex-cluster the other 31 were PF 0.99 (net flat), and the blended figure has decayed toward that ex-cluster reality on every subsequent milestone (PF 1.37 → 1.23 → 1.01). Not a sector cluster (tickers spanned energy/healthcare/financials/tech/consumer) — a regime/timing correlation, a variant of the Day 100 no-correlation-awareness gap. **Retirement mechanism:** `live_signals.get_momentum_signals()` no longer appends the `B_revised_rr` gate (Day 112 code change); the 67 open + 15 pending positions wind down by their own exit rules over the following ~3 weeks, and Path B's final stats remain visible in `daily_job.py --report` as a frozen historical record. `check_sr_gate()` is left intact in the code for a possible future **corrected-S&R parallel run** decided alongside the Golden Rule 53 `_pivot_sr()` fix — that would be a new, separately pre-registered experiment, not a resumption of this one. Path A (`variant='A_frozen'`) is unaffected and continues toward its own 100-trade bar; MR (broad) continues as the confirmed benchmark.
...
## 9b. HUB-65 — Parallel Mean-Reversion Universe Experiment (Day 98)

**Not a change to Section 9 above.** The broad MR arm (`variant='A_frozen'`) continues completely unchanged, accumulating toward its own 100-trade bar exactly as before. HUB-65 is a new, separately pre-registered, honestly-tracked experiment running *alongside* it — **same entry gate, same exit rule, same everything in Section 9 — only the candidate universe differs.**

| Rule | Broad MR (unchanged) | HUB-65 (new) |
|------|------------------------|----------------|
| Entry gate (RSI(2)<10, price>200SMA, price>$10, 20d ADV>$25M) | Section 9, unchanged | **Identical** — same `detect_mr_signal()` call, no logic difference |
| Exit rule (RSI(2)>70 OR 10-day max, 5% stop) | Section 9, unchanged | **Identical** |
| Candidate universe | Dynamic ~150-ticker TradingView liquid-universe scan | Fixed 64-ticker curated watchlist (`mean_reversion.HUB_UNIVERSE`) — semis, uranium/nuclear, fintech, China-EV, thematic ETFs, sourced from a sibling project's own screener research, captured 2026-07-22 |
| Ledger tag | `variant='A_frozen'` | `variant='mr_hub65'` — **`variant` here encodes UNIVERSE, not gate-experiment**, a different axis than momentum's Path A/B use of the same column (Golden Rule 38); documented explicitly in `ledger.py`'s schema comment so a future read of this column doesn't assume one meaning across both systems |
| Confirmation bar | 100 trades (unchanged, Section 10) | 100 trades, tracked independently |
| Cooldown/active-position tracking | Independent per variant | Independent per variant — verified via a throwaway-DB isolation test before shipping |

**Backtest caveat (stated loudly, not quietly):** `backend/backtest/backtest_hub_mr.py` ran the unchanged MR engine against HUB-65 (1,940 trades, PF 1.2574, Sharpe 1.5278, block-bootstrap p=0.0311, 76 distinct entry-months) — but this number is **not comparable** to the project's own survivorship-free baseline (PF 1.16, Sharpe 1.30, random 400-ticker sample). HUB-65 is a 2026 watchlist of names selected *because* they already look strong (a milder version of the exact survivorship bias Golden Rule/Fable review already found and corrected once) — a cleaner backtest number here reflects curation, not necessarily a better edge. The forward-test track is the real test, same as every other system in this document (Section 10's own philosophy).

**Why this isn't a Golden Rule 18 violation:** the broad MR arm's frozen thresholds and accumulated count are completely untouched. HUB-65 is a brand-new, separately-tracked hypothesis test ("does curation help, or just concentrate risk on the same underlying edge") pre-registered before any of its own trades existed — same legitimate pattern as Path B (Section 8b) and the Day 79/80 MR liquidity re-test.
