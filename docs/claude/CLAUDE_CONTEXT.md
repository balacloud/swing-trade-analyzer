...
> **Last Updated:** Day 107 — end of day (August 14, 2026)
...
|-------|-------|
| Current Day | 107 |
| Version | v4.58 unchanged (Backend v2.46 unchanged, Frontend v4.53 unchanged) — no live app code touched, research/backtest-only session |
| Latest Status | PROJECT_STATUS_DAY107_SHORT.md |
| Latest Issues | KNOWN_ISSUES_DAY107.md |
| Latest API | API_CONTRACTS_DAY105.md — no API changes Day 106 or 107, still the latest contract file |
| Focus | **Forward-testing accumulation remains the SOLE priority for the 4 live automated tracks — none touched Day 107 either.** Session built a code-audited, 3-source-cross-checked "STA vs. 100 Years of Trading Principles" comparison Artifact (7/9 categories match, 1 partial, 1 gap), then ran a backtest-only research spike testing the 1 partial + 1 gap directly (Config F/G, purely additive, zero live-code impact) — neither shows a credible edge yet (volume confirmation's tested design was too strict, 93% trade cut; dual momentum tested clean but near-inert). Both stay parked on ROADMAP Priority #11 with real data now behind the "not yet." Side-finding: SimFin's ticker universe has drifted since Day 79, so old seeded backtests won't reproduce exactly (new Golden Rule 45); a real, unfixed UI bug found (Fear & Greed gauge's 3rd stale threshold copy in App.jsx). Also deep-validated Path B forward-test data and re-ran `/watchlist-report` (found + fixed a real cross-provider stock-split data mismatch, new Golden Rule 46). |
...
**Forward-testing accumulation is the SOLE priority as of Day 92, unchanged Day 93-107** — user explicitly parked every other item below until 100 trades/system (raised from 50) are logged, now across FOUR tracks. Do not propose or start any of items 2+ unless the user raises it first — this is stricter than the Day 87 feature freeze it supersedes. (The Day 93 Sectors/Context tab work, Day 94's Sector Rotation fix + README audit, Day 95's launchd schedule/timezone fix, Day 96's provider-reliability overhaul + PERSONA.md, Day 98's HUB-65 track, Day 99's partial-bar ledger contamination fix, Day 100's monitoring/documentation session, Day 101's Sub-Industry Watch feature, Day 103's SRPS investigation + Sector Pullback Screener, Day 104's `/watchlist-report` skill build, Day 105's Sub-Sector Pullback Screener + Sectors-tab staleness fix, Day 106's Scan tab labeling fix + STA Verdict column + self-description correction, and Day 107's trading-principles comparison Artifact + backtest-only research spike were explicitly scoped as independent of this freeze — pure display/UI/docs/ops/infra, a genuinely new separately-tracked experiment (that in SRPS's case failed its own backtest and was never built live), a data-integrity fix to existing infrastructure, a user-requested extension of an already-informational screener, a standalone Claude Code skill that touches no STA app code at all, or (Day 107) backtest-only research explicitly confirmed with the user as zero-live-impact before starting — not new exceptions to the pattern. Path B and HUB-65 are both genuinely new experiments, not freeze exceptions, since neither touches an existing track's count. Day 97's IBKR research/plan is explicitly parked by the user's own choice — do not start Phase 1 unless raised again. Day 99's "Quick" momentum holding-period idea was scoped/planned but the user stopped before approval — nothing built, needs re-planning from scratch if raised again. Day 100's scheduled breakout-alert watcher idea (ROADMAP priority #15) is explicitly idea-only — needs its own design session before any build, not started.)

1. **Let paper trading accumulate** — SOLE FOCUS, now across 4 tracks, on a *corrected* ledger (Day 99 partial-bar fix — see Golden Rule 39). **Track numbers move fast between sessions — always pull live via `daily_job.py --report`, never quote a status doc.** As of last live check (2026-08-13, job last ran 2026-08-12): **Path A** Momentum 35 open/26 closed (42.31% WR, PF 1.111); **Path B** Momentum 92 open/30 closed (60.0% WR, PF 1.468); **MR** (broad) 7 open/**84 closed (78.57% WR, PF 4.5255** — closest to its bar by a wide margin, same expected-cooling read as prior sessions applies); **MR (HUB-65)** 3 open/29 closed (51.72% WR, PF 1.4306). Confirmation bar is **100 trades/system** for all four (`PAPER_TRADING_PREREGISTRATION.md`). MR (broad) is closest to the bar at 84/100 — plausibly clears it within the next few sessions. The job's `launchd` schedule was corrected Day 95 (now 17:30 ET). `/sta-start` warns automatically if the launchd job goes stale (>3 days). If a Force Run "looks like nothing happened," don't assume it's a quiet day — re-run in the foreground and read stdout before trusting the aggregate UI. Check progress via the Forward Test tab's status panel or `venv/bin/python paper_trading/daily_job.py --report`.
...
7c. *(parked, Day 107)* **Fear & Greed gauge uses a third, stale threshold-band copy** — found while three-pass verifying the trading-principles audit. `App.jsx`'s sentiment gauge bar independently hardcodes old 60/35 color thresholds, different from both the correct bands driving the real verdict and a second, already-known dead copy in `backend.py`. Real, live, user-visible; not fixed.
...
17. *(parked, offered not requested, Day 106)* **Canadian Analyze Page — real scope now uncertain, needs a proper recheck.** Long-documented as broken (Analyze page needs a data-source redesign for `.TO` tickers); a Day 106 spot-check (CMG.TO) worked correctly on both Simple Checklist and Full Analysis, verified against the backend directly. Not exhaustively retested across other Canadian tickers/exchanges, so not marked resolved — but the "confirmed broken" framing in README/KNOWN_ISSUES is no longer accurate either. Worth a real multi-ticker recheck before closing or reconfirming.
