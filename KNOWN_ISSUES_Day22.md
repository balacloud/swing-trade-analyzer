...
### CRIT-2: TradingView Returns OTC Stocks
| Field | Value |
|-------|-------|
| Status | 🟡 IN PROGRESS |
| Affected | Scan Market tab |
| Since | Day 18 (originally "returns HTML"), Day 20 (OTC bug) |
| Root Cause | Exchange filter not applied correctly in query chain |
| Impact | Scanner unusable - returns pink sheets instead of NYSE/NASDAQ |

**Diagnostic Finding (Day 20):**
```python
# Library DOES return proper exchange values:
# NYSE: 409, NASDAQ: 337, AMEX: 216, CBOE: 37, OTC: 1

# This query works in isolation:
Query().set_markets('america').select('name', 'exchange').limit(20)
# Returns: SPY (AMEX), QQQ (NASDAQ), TSLA (NASDAQ), NVDA (NASDAQ)...

# But endpoint returns: SCGLF, SBGOF, TRAUF (all OTC)
# Bug is in how filter is chained in scan_tradingview()
```

**Fix Approach:**
1. Run diagnostic query with filter in isolation
2. Compare working query structure vs endpoint query
